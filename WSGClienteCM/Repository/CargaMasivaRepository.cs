...
namespace WSGClienteCM.Repository
{
    public class CargaMasivaRepository : ICargaMasivaRepository
    {
        ...
        public async Task<ResponseViewModel> InsertDetail(List<ClientBindingModel> items, DbConnection cn, DbTransaction trx)
        {
            ...
            try
            {
                ...
                foreach (ClientBindingModel item in items)
                {
                    ...
                    parameters.Add(new OracleParameter("P_ADDRESSTYPE", OracleDbType.Char, item.EListAddresClient[0].P_SRECTYPE, ParameterDirection.Input));
                    ...
                    parameters.Add(new OracleParameter("P_SEMAILTYPE", OracleDbType.Char, item.EListEmailClient[0].P_SRECTYPE, ParameterDirection.Input));
        ...
        public async Task<ResponseViewModel> updateState(string jiraCode, string jiraStatus, string derivationArea, string denvio, string dateFired, string attendedDate)
        {
        ...
        public async Task<ResponseViewModel> updateStateObservation(string jiraCode, string jiraStatus)
        {
            ...
            try
            {
                ...
                using (OracleDataReader dr = (OracleDataReader)await _connectionBase.ExecuteByStoredProcedureVTAsync(string.Format("{0}.{1}", Package4, "UPD_TICKET_WH_OBSERVATION"), parameter, ConnectionBase.enuTypeDataBase.OracleVTime))
                {
                    response.P_COD_ERR = P_NCODE.Value.ToString();
                    response.P_MESSAGE = P_SMESSAGE.Value.ToString();
                }
